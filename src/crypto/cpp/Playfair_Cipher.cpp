...
// playfair_cipher::playfair_cipher(std::string key_value = "") : basic_cripto() {}
...
std::pair<int, int> playfair_cipher::findPosition(char c) {
    for (int i = 0; i < matrix_size; i++) {
        for (int j = 0; j < matrix_size; j++) {
            if (matrix[i][j] == c) {
                return {i, j};
...
std::string playfair_cipher::get_key() const {
    return "key=" + key;
};
        
// Установка ключа из строки
bool playfair_cipher::set_key(const std::string& key_str) {
...
playfair_cipher::playfair_cipher(std::string key_value) {
    if (key_value.empty()) {
...
std::string playfair_cipher::encode(const std::string message)  {
...
std::string playfair_cipher::decode(const std::string message)  {
...
std::pair<std::string, std::string> playfair_cipher::encode_with_key(const std::string message)  {
...
std::pair<std::string, std::string> playfair_cipher::decode_with_key(const std::string message, const std::string key_str)  {
